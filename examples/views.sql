...
SELECT
  orders.id "Order id",
  users.id "User id",
  sum(price * pto.quantity) "Total price"
FROM
  users
...
SELECT
  sum("Total price") "Total user spending"
FROM
  orders_with_price;
...
-- говорим что надо заполнить материализованные данные
-- WITH NO DATA если сначала не хотм туда ложить данные
REFRESH MATERIALIZED VIEW orders_with_price_m;

-- пока вы обновляетесь то к таблицам не постучатся
REFRESH MATERIALIZED VIEW CONCURRENTLY orders_with_price_m;
...
SELECT
  ...
  sellers.address,
  sellers.phone,
  sellers.created_at become_seller_at,
  count(products.*)
FROM
  users
